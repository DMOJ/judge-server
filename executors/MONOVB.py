...
class Executor(ResourceProxy):
    def __init__(self, problem_id, source_code):
        ...
        source_code_file = self._file('%s.vb' % problem_id)
        self.name = self._file('%s.exe' % problem_id)
        with open(source_code_file, 'wb') as fo:
            fo.write(source_code)
        csc_args = [env['runtime']['mono-vbnc'], source_code_file]
...
def initialize():
    if 'mono-vbnc' not in env['runtime'] or 'mono' not in env['runtime']:
        return False
    if not os.path.isfile(env['runtime']['mono-vbnc']):
        return False
    return test_executor('MONOVB', Executor, '''\
Imports System
